...
from gpt2.src.slurp_alt_gpt_encoder import gpt_encode
from alt_gpt_decoder import gpt_decode
from pipeline.slurp_altencoder import pipeline_encode
...
# ]
TEST_FOLDERS = ["gatsby_intro", 
    "orwell_af", "woolf_essays", "gpt_med_w8_v5"
]

WINDOW = 10
TOP_K = 5
progressf = "SLURP/failures/slurp_runprogressW" + str(WINDOW) + "K" + str(TOP_K) + ".txt"
failf = "slurpfailures.txt"
...
def run_gpt():
    with open(progressf, 'a+') as pf:
        for folder in TEST_FOLDERS:
            ...
            pf.flush()
            newfolderstr = "SLURP_MgptW" + str(WINDOW) + "K" + str(TOP_K)
    ...
    with open(progressf, 'a+') as pf:
        for model in PIPELINE_MODELS:
            ...
            for folder in TEST_FOLDERS:
                ...
                pf.flush()
                newfolderstr = "SLURP_M" + model + str(WINDOW) + "K" + str(TOP_K)
                newfolder = "tests/" + folder + "/" + newfolderstr
                os.mkdir(newfolder)
                shutil.copy("tests/" + folder + "/" + folder + ".txt", newfolder)
